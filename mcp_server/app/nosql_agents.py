        ...
        try:
            if hasattr(self.bedrock_client, 'bedrock_client') and self.bedrock_client.bedrock_client:
                ...
                response = await asyncio.wait_for(
                    asyncio.to_thread(
                        self.bedrock_client.bedrock_client.converse,
                        ...
                        inferenceConfig={
                            "maxTokens": 4000,  # Increased for complex architecture responses
                            "temperature": 0.1,
                            "topP": 0.9
                        }
                    ),
                    timeout=90.0  # 90 second timeout for complex architecture analysis
                )
            ...
            return {"success": False, "error": "Bedrock request timed out after 90 seconds"}
        except Exception as e:
            return {"success": False, "error": f"Bedrock call failed: {str(e)}"}
        except Exception as e:
...
class NoSQLArchitectureAgent(BaseNoSQLAgent):
    ...
    async def analyze(self, request: Dict[str, Any], context: Dict[str, Any] = None) -> AgentResult:
        ...
        try:
            ...
            prompt = f"""
            Design NoSQL architecture for: {request.get('application', 'Unknown')}
            Database: {primary_db}, Cache: {primary_cache}
            Multi-Region: {requirements.get('multi_region', False)}, HA: {requirements.get('high_availability', True)}
